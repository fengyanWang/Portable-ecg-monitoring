...
//OLED控制用函数
void OLED_WR_Byte(unsigned dat,unsigned cmd);  //OLED写入一个字节函数（命令或者数据）
...
void OLED_Set_Pos(unsigned char x, unsigned char y);//坐标设置函数
void OLED_ShowCHinese(u8 x,u8 y,u8 no);//显示汉字
void OLED_DrawBMP(unsigned char x0, unsigned char y0,unsigned char x1, unsigned char y1,unsigned char BMP[]);//显示显示BMP图片128×64
void Delay_50ms(unsigned int Del_50ms);
void Delay_1ms(unsigned int Del_1ms);
void fill_picture(unsigned char fill_Data);//OLED显示图片
